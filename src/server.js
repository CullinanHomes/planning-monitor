...
const fs = require('fs');
// Use the persistent Railway volume mounted at /data so leads survive restarts and redeploys.
// Falls back to /tmp only if no volume exists (e.g. running locally).
const DB_PATH = process.env.DB_PATH || (fs.existsSync('/data') ? '/data/leads.db' : '/tmp/leads.db');
...
app.post('/api/classify', auth, async function(req, res) {
  ...
  try {
    var response = await client.messages.create({ model: 'claude-sonnet-4-6', max_tokens: 4000, messages: [{ role: 'user', content: prompt }] });
  ...
  for (var i = 0; i < leads.length; i++) {
    ...
    try {
      var result = await dbRun(
        'INSERT OR IGNORE INTO leads (ref,lpa,address,postcode,applicant,agent,description,date_submitted,date_scraped,app_type,is_new_application,contract_value_min,contract_value_max,floor_area_sqft,planning_likelihood,planning_notes,priority_score,no_agent,signals,portal_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        [l.ref||l.address,l.lpa,l.address||'',l.postcode||'',l.applicant||'',l.agent||'',l.description||'',l.date_submitted||'',now,l.app_type||'other',l.is_new_application?1:0,l.contract_value_min||0,l.contract_value_max||0,l.floor_area_sqft||0,l.planning_likelihood||0,l.planning_notes||'',l.priority_score||0,l.no_agent||0,JSON.stringify(l.signals||[]),l.portal_url||'']
      );
...
app.get('/api/leads', auth, async function(req, res) {
  ...
  var params=[];
  if (view==='active'){sql+=' AND (archived IS NULL OR archived=0)';}
  ...
  var countParams=[];
  if (view==='active'){countSql+=' AND (archived IS NULL OR archived=0)';}
...
app.get('/api/stats', auth, async function(req, res) {
  ...
  res.json({total:total,thisWeek:thisWeek,noAgent:noAgent,pipeline:pipeline,archived:archived});
});
...
app.post('/api/leads/:id/contact-details', auth, async function(req, res) {
  await dbRun('UPDATE leads SET contact_name=?, contact_phone=?, contact_email=?, contact_address=? WHERE id=?',
    [req.body.contact_name||'', req.body.contact_phone||'', req.body.contact_email||'', req.body.contact_address||'', req.params.id]);
  res.json({ok:true});
});

app.get('/api/export', auth, async function(req, res) {
  var leads=await dbAll('SELECT * FROM leads ORDER BY date_submitted DESC');
  var fields=['lpa','address','postcode','applicant','agent','description','app_type','contract_value_min','contract_value_max','floor_area_sqft','planning_likelihood','planning_notes','priority_score','no_agent','date_submitted','ref','contact_name','contact_phone','contact_email','contact_address','contacted','archived','notes'];
