...
db.serialize(function() {
  ...
  // Pending post: a lead whose letter has been generated/printed and is waiting to be posted —
  // sits below the main list on the dashboard as its own separated section.
  db.run('ALTER TABLE leads ADD COLUMN pending_post INTEGER DEFAULT 0', function(e) { /* ignore if column already exists */ });
  db.run("ALTER TABLE leads ADD COLUMN pending_post_date TEXT DEFAULT ''", function(e) { /* ignore if column already exists */ });
...
app.get('/api/land', auth, async function(req, res) {
  var area=req.query.area, council=req.query.council, type=req.query.type, status=req.query.status, search=req.query.search, due=req.query.due;
...
app.get('/api/leads', auth, async function(req, res) {
  ...
  sql+=' ORDER BY (CASE WHEN pending_post=1 THEN 1 ELSE 0 END) ASC, '+(sort==='value'?'contract_value_max DESC':sort==='planning'?'planning_likelihood DESC':sort==='score'?'priority_score DESC':sort==='contactedDate'?'contacted_date DESC':'date_scraped DESC, date_submitted DESC');
  sql+=' LIMIT ? OFFSET ?';
...
app.get('/api/stats', auth, async function(req, res) {
  ...
  var pendingPost=(await dbGet('SELECT COUNT(*) as n FROM leads WHERE pending_post=1')).n;
  ...
  var newCount=(await dbGet("SELECT COUNT(*) as n FROM leads WHERE (archived IS NULL OR archived=0) AND (stage IS NULL OR stage='new')")).n;
  var winRate=(stageCounts.won+stageCounts.lost)>0?Math.round(stageCounts.won/(stageCounts.won+stageCounts.lost)*100):null;
  // "Contacted" only means outreach went out — it says nothing about whether the applicant got
  // back to us. A real response is a lead that moved past 'contacted' into active engagement
  // (interested/quoted/won). 'lost' is deliberately excluded — it's also used to mark leads that
  // went cold with no reply at all, so including it would overstate how many people responded.
  var respondedCount = stageCounts.interested + stageCounts.quoted + stageCounts.won;
  var responseRate = contacted>0 ? Math.round((respondedCount/contacted)*100) : 0;
  // Per-council breakdown so response quality can be compared across LPAs, not just overall.
  var lpaRows=await dbAll(
    "SELECT lpa,"+
    " COUNT(*) as total,"+
    " SUM(CASE WHEN contacted=1 THEN 1 ELSE 0 END) as contacted,"+
    " SUM(CASE WHEN stage='interested' THEN 1 ELSE 0 END) as interested,"+
    " SUM(CASE WHEN stage='quoted' THEN 1 ELSE 0 END) as quoted,"+
    " SUM(CASE WHEN stage='won' THEN 1 ELSE 0 END) as won,"+
    " SUM(CASE WHEN stage='lost' THEN 1 ELSE 0 END) as lost"+
    " FROM leads WHERE (archived IS NULL OR archived=0) GROUP BY lpa ORDER BY total DESC"
  );
  var lpaBreakdown=lpaRows.map(function(r){
    var respondedN=r.interested+r.quoted+r.won;
    return {
      lpa: r.lpa||'Unknown',
      total: r.total,
      contacted: r.contacted,
      outreachRate: r.total>0?Math.round((r.contacted/r.total)*100):0,
      respondedCount: respondedN,
      responseRate: r.contacted>0?Math.round((respondedN/r.contacted)*100):0,
      won: r.won,
      lost: r.lost
    };
  });
  res.json({total:total,thisWeek:thisWeek,noAgent:noAgent,pipeline:pipeline,archived:archived,pendingPost:pendingPost,contacted:contacted,contactRate:contactRate,respondedCount:respondedCount,responseRate:responseRate,avgDaysToContact:avgDaysToContact,stageCounts:stageCounts,newCount:newCount,winRate:winRate,lpaBreakdown:lpaBreakdown});
});
...
app.post('/api/leads/:id/pending-post', auth, async function(req, res) {
  var flag = req.body.pending_post ? 1 : 0;
  var date = flag ? (new Date().toISOString().split('T')[0]) : '';
  await dbRun('UPDATE leads SET pending_post=?, pending_post_date=? WHERE id=?',[flag, date, req.params.id]);
  res.json({ok:true});
});
...
app.get('/api/export', auth, async function(req, res) {
  ...
  var fields=['lpa','address','postcode','applicant','agent','description','app_type','contract_value_min','contract_value_max','floor_area_sqft','planning_likelihood','planning_notes','priority_score','no_agent','date_submitted','ref','contact_name','contact_address','agent_address','contacted','contacted_date','stage','stage_date','lost_reason','pending_post','pending_post_date','archived','notes'];
