      ...
      await dbRun(
        `INSERT INTO land_sites
          (type,address,area,council,letter1_date,letter2_date,letter3_date,letter4_date,
           doorknock1_date,doorknock2_date,status,responded,response_date,outcome,notes,date_added)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        [r.type||'', r.address, r.area||'', r.council||'',
         ...
         r.notes||'', r.date_added||today]
      );
      imported++;
    } catch(e) { errors.push({ address: r.address, error: e.message }); }
  }
  res.json({ imported: imported, skipped: skipped, errors: errors });
});
