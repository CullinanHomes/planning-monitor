...
async function scrapeElmbridge() {
  ...
  } catch (e) { console.error('Elmbridge scrape error:', e.message); }
  return results;
}
...
async function scrapeRichmond() {
  const results = [];
  try {
    const { data } = await axios.get('https://www2.richmond.gov.uk/PlanningApplications/search.aspx', { timeout: 15000, headers: { 'User-Agent': 'Mozilla/5.0' } });
    const $ = cheerio.load(data);
  ...
  } catch (e) { console.error('Richmond scrape error:', e.message); }
  return results;
}
...
async function scrapeMerton() {
  const results = [];
  try {
    const { data } = await axios.get('https://planning.merton.gov.uk/Northgate/PlanningExplorer/GeneralSearch.aspx', { timeout: 15000, headers: { 'User-Agent': 'Mozilla/5.0' } });
    const $ = cheerio.load(data);
  ...
  } catch (e) { console.error('Merton scrape error:', e.message); }
  return results;
}
  ...
  for (const chunk of chunks) {
    try {
      const response = await client.messages.create({
        ...
        max_tokens: 2000,
        messages: [{ role: 'user', content: `You are a property development lead qualifier for a design-and-build contractor in Surrey/SW London. For each planning application below, return a JSON array with one object per application. Rules: app_type: one of "large_extension","selfbuild","conversion","loft_complex","other". est_value_min and est_value_max: estimated BUILD cost in GBP integers. priority_score: 0-100. no_agent: 1 if no architect or agent name visible. signals: array of up to 3 short strings. qualified: true if est_value_min >= 150000. Applications: ${JSON.stringify(chunk.map(a => ({ ref: a.ref, description: a.description, agent: a.agent, address: a.address })))} Return ONLY a JSON array, no markdown.` }]
      });
      const text = response.content[0].text.replace(/```json|```/g, '').trim();
      const results = JSON.parse(text);
      results.forEach((r, i) => { if (chunk[i]) classified.push({ ...chunk[i], ...r, signals: JSON.stringify(r.signals || []) }); });
    } catch (e) {
...
async function saveLeads(leads) {
  ...
  for (const l of leads) {
    const m = l.address && l.address.match(/[A-Z]{1,2}\d{1,2}[A-Z]?\s?\d[A-Z]{2}/i);
    const lrUrl = m ? `https://search.landregistry.gov.uk/app/search#?query=${encodeURIComponent(l.address)}` : '';
    try {
      const result = await dbRun(`INSERT OR IGNORE INTO leads (ref,lpa,address,applicant,agent,description,date_submitted,date_scraped,app_type,est_value_min,est_value_max,priority_score,no_agent,signals,land_registry_url,portal_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
...
async function runScrape() {
  ...
  for (const { name, fn } of [{ name:'Elmbridge', fn:scrapeElmbridge },{ name:'Richmond', fn:scrapeRichmond },{ name:'Merton', fn:scrapeMerton }]) {
    try {
      const raw = await fn();
...
app.get('/api/export', auth, async (req, res) => {
  ...
  const csv = new Parser({ fields }).parse(leads);
