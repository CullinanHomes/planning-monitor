...
app.get('/api/stats', auth, async function(req, res) {
  ...
  res.json({total:total,thisWeek:thisWeek,noAgent:noAgent,pipeline:pipeline,archived:archived,contacted:contacted,contactRate:contactRate,avgDaysToContact:avgDaysToContact});
});
...
app.post('/api/leads/:id/contact', auth, async function(req, res) {
  ...
  if (contacted) {
    ...
    await dbRun('UPDATE leads SET contacted=1, contacted_date=?, notes=? WHERE id=?',[date, req.body.notes||'', req.params.id]);
  } else {
    await dbRun("UPDATE leads SET contacted=0, contacted_date='' WHERE id=?",[req.params.id]);
  }
  res.json({ok:true});
});
...
app.get('/api/export', auth, async function(req, res) {
  ...
  var fields=['lpa','address','postcode','applicant','agent','description','app_type','contract_value_min','contract_value_max','floor_area_sqft','planning_likelihood','planning_notes','priority_score','no_agent','date_submitted','ref','contact_name','contact_address','agent_address','contacted','contacted_date','archived','notes'];
