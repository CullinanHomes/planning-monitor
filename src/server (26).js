...
db.serialize(function() {
  ...
  // Migration: pending post — a lead whose letter has been generated/printed and is waiting
  // to be posted. Sits below the main list on the dashboard as its own separated section.
...
app.get('/api/leads', auth, async function(req, res) {
  ...
  // Pending-post leads sink to the bottom of the results, below everything else, regardless
  // of the chosen sort — they're "done" from a working-list point of view, just not yet posted.
...
app.get('/api/stats', auth, async function(req, res) {
  ...
  var pendingPost=(await dbGet('SELECT COUNT(*) as n FROM leads WHERE pending_post=1 AND (archived IS NULL OR archived=0)')).n;
  ...
  res.json({total:total,thisWeek:thisWeek,noAgent:noAgent,pipeline:pipeline,archived:archived,contacted:contacted,contactRate:contactRate,pendingPost:pendingPost,avgDaysToContact:avgDaysToContact,stageCounts:stageCounts,winRate:winRate});
});
